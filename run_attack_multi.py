...
from model.ensemble_model import Ens
...
def parse_args():
    ...
    parser.add_argument('--model_1', required=True, help='the model for black-box attack')
    parser.add_argument('--model_2', required=True, help='the model for black-box attack')
    parser.add_argument('--model_3', required=True, help='the model for black-box attack')
    parser.add_argument('--model_4', required=True, help='the model for black-box attack')
...
def main():
    ...
    print(f'surrogate model: {configs.model_1}, {configs.model_2}, {configs.model_3}, {configs.model_4}'
          f'\tattack method: {configs.method}')

    ens_model = Ens([models[configs.model_1],
                     models[configs.model_2],
                     models[configs.model_3],
                     models[configs.model_4]])

    for idx, (data, label) in enumerate(tqdm(test_loader)):
        n = label.size(0)
        data, label = data.to(device), label.to(device)
        # get attack
        attack_method = get_attack(configs, model=ens_model, device=device)
