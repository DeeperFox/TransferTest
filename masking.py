...
def apply_heatmap(cam):
    ...
    heatmap = cm.jet(cam.cpu())[..., :3]  # 使用colormap将归一化的CAM值转化为热力图
...
def get_GradCAM(model, images, labels, upsample=None):
    ...
    target_layer = model[-1].layer4[-1].conv2  # 获取模型中的目标层
    # target_layer = model.layer4[-1].conv2  # 获取模型中的目标层
    ...
    # print("activations[0] requires grad:", activations[0].requires_grad)
...
class CAMMaskSingleFill(Attack):  # 定义 CAMMaskSingleFill 类，继承 Attack 类
    ...
    def forward(self, images, labels, fill_image=None):  # 定义前向传播方法
        ...
        if fill_image is not None:
            ...
            # print("fill_image shape:", fill_image.shape)
